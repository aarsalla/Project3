  ...
  d3.json("/dict")
    .then(data => {
      ...
      d3.json(url).then(function(data) {
        ...
        Plotly.plot("graph0", data4, layout);
      });
  ...
  d3.json("/dict").then(data => {
    ...
    switch (dataset) {
      case ReportedDate[3]:
        ...
        var apiKey = "REHgZFPuj_3cxTxuwvsn";
        console.log(surprise[0]);
        var url = `https://www.quandl.com/api/v3/datasets/EOD/${stock}?start_date=${date_q1}&end_date=${month_q1}&api_key=REHgZFPuj_3cxTxuwvsn`;
      ...
      case "dataset2":
        ...
        d3.json(url).then(function(data) {
          ...
          for (i = 0; i < dates.length; i++) {
            x.push(dates[i]);
            y.push(closingPrices[i]);
          }
          console.log({y});
          updatePlotly(x, y);

        });
        //  y = (d3.json(url).then(function(data){
        //     var closingPrices = unpack(data.dataset.data, 1);
        //   y = closingPrices
        // return (y)}))
        //x1 =[]
        //x.then(function(result){for (i=0; i<result.length; i++){
        // x1.push(result[i])}})
...
function updatePlotly(newx, newy) {
  const LINE = document.getElementById("graph0");
  console.log({ newx });
